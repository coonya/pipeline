...
def post_mapping(sample)
	cmd = []

	cmd.append(bwa.sampe(sample))
	cmd.append(picard.MergeBamAlignment(sample))
	cmd.append(picard.dedup(sample))
	cmd.append(gatk.IndelRealigner(sample))
	cmd.append(gatk.CountCovariates(sample))
	cmd.append(gatk.TableRecalibration(sample))
	cmd2 = '\n'.join(cmd)
...
wname = '%s/01.BaseCall.%s.sh' % (log_dir, wait_id1)
wfile = open(wname,'w')
...
for x in sample_list:
	...
	wname = '%s/02.SamToFastq.%s.%s.sh' % (log_dir, x, wait_id1)
	wfile = open(wname,'w')
...
for x in sample_list:
	...
	wait_key = 'SamToFastq_%s' % x
	wait_id = qsub_id[wait_key]

	wname = '%s/03.bwa_aln.%s.%s.sh' % (log_dir, x, wait_id1)
	wfile = open(wname,'w')
	wfile.write('%s\n' % pbs_header2(cpu_no['aln'], 'no', wait_id, mapping(x)))
	wfile.close()
...
for x in sample_list:
	...
	wait_key = 'bwa_aln_%s' % x
	wait_id = qsub_id[wait_key]

	wname = '%s/04.post_mapping.%s.%s.sh' % (log_dir, x, wait_id1)
	wfile = open(wname,'w')
	wfile.write('%s\n' % pbs_header2(cpu_no['postmapping'], 'no', wait_id, post_mapping(x)))
	wfile.close()
	...
	qsub_id[qsub_key] = wait_id1
	#qsub_list.append('qsub %s' % wait_id1)
